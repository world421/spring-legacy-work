...
@Repository("spring")
@RequiredArgsConstructor // template에 주입됨 ! 
public class ScoreSpringRepository implements IScoreRepository {
	...
	public Score findByStuNum(int stuNum) {
		...
		try {
			// queryForObject 는 조회결과가 없을 시 예외가 발생합니다.!! 
			return template.queryForObject(sql, new ScoreMapper(), stuNum);
		} catch (Exception e) {
			 // 조회 결과가 없다면 catch 문이 실행 
			return null;
		}
		//queryForObject 통해서 학생 한명의 정보를 가져옴!
		//sql 을 실행해서 조회된 결과가 없으면 오류 ! - > EmptyResultDataAccessException -> exception 으로 처리
	}
	...
	public void modify(Score modScore) {
		...
		template.update(sql,modScore.getKor(),modScore.getEng(),modScore.getMath(),modScore.getTotal(),
				modScore.getAverage(),String.valueOf(modScore.getGrade()),modScore.getStuNum());
