...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>  
    ...
    <script>
        //브라우저 창이 로딩이 완료된후 실행할것을 보장하는 이벤트
        window.onload = function(){
            //사용자가 페이지 관련버튼 을 클릭했을 때 (이전, 다음, 1,2,3 ....)
            //a태그의 href에다가 각각 다른 url 을 작성해서 요청을 보내기가 귀찮다 
            // 클릭한 버튼이 무엇인지 를 확인해서 그 버튼에 맞는 페이지 정보를
            // ㅈ자바스크립트로 끌고와서 요청을 보내주겠다.
            document.getElementById('pagenation').addEventListener('click', e => {
                ...
                //현재 이벤트가 발생한 요소(버튼)의
                //data - pagenum 의 값을 얻어서 변수에 저장.
                //data- 으로 시작하는 속성값을 dataset 프로퍼티로 쉽게 끌고올수있습니다. 
                // data-  태그에 어떤값을 붙여놓고 나중에 자바스크립트에서 끌고오기 위해 사용함 ! 
                const value = e.target.dataset.pagenum;

                //페이지 버튼들을 감싸고 있는 form 태그를 지목하여
                // 그 안에 숨겨져 있는 input 태그의 value에
                // 위에서 얻은 data-pegenum의 값을 삽입한 후 submit
