        ...
        window.onload = function(){
            ...
            document.getElementById('replyRegist').onclick = () => {
                ...
                const reqObj = {
                    method: 'post',
                    headers : {
                        'Content-Type' : 'application/json' //제이슨 타입으로 ~! 보내줘야지
                    } ,
                ...
                .then(res => res.text()) //promise객체 then 을 부름  
                .then(data => {// regSecc 가 올거임
                    console.log('통신 성공!' , data) //등록 성공 했다면 , 다음 등록을 위해 입력창을 비워주자
                    document.getElementById('reply').value ='';
                    document.getElementById('replyId').value ='';
                    document.getElementById('replyPw').value ='';

                    //등록완료후 댓글 목록 함수를 호출해서 비동기식으로 목록 표현하기
                    getList(1,true);
                });                 
            }//댓글 등록 이벤트 끝.
            ...
            document.getElementById('moreList').onclick = () =>{
                /*
                 - false 주는 이유 ?
                   더보기니까 ! 누적해서 보여줘야하니까 -> 초기화하면안됨 ~! 
                */
                getList(++page, false) ; // 올리고보내랏 선증감후연산 
            }
            ...
            // 페이지가 비동기방식이기때문에 페이지가 그대로 계속 머물면서 댓글이 밑에 쌓입니다.
            // 때에 따라서는 댓글을 계속 누적시키는 것이 아닌, 화면을 초기화하고 다시 새롭게 보여줘야 할 때가 있다.~!
            // reset -> true : 페이지를 리셋해서 새롭게 그려내기, reset -> false:누적해서 쌓기 
            function getList(pageNum, reset){
                console.log('getList() 호출됨 ~ ');
                strAdd = ''; //새로운 내용 을 작성하기 위해 변수의 값을 비우기
              
                const bno = '${article.bno}' ; //현재 게시글 번호
            

                //get 방식으로 댓글 목록 요청 (비동기)
                ...
                .then(data =>{
                    ...
                    if(reset ){ //reset 의 결과가 true라면
                        while($replyList.firstChild){ //얻어낼자식이없으면 null () false겟지!
                    ...
                    //★더보기 버튼 배치 판단
                    //예시 ) 페이지 위치 3페이지고 5개씩 보여줄건데 총게시물 27개면 더보기이썽야징
                    //현재 페이지 번호  X 이번 요청으로 받은 댓글 수 > 전체 댓글수 -> 더보기 없어도됨!
                    ...
                    // id가 replyList 라는 div 영역에 문자열 형식으로 모든 댓글을 추가.
                    // insertAdjacentHTML : 인접한 곳에 HTML 을 삽입하는 함수.
                    //(position 문자열, 문자열 형태의 HTML)

                    if(!reset){
                        $replyList.insertAdjacentHTML('beforeend', strAdd); //매개값으로 값을 2개 받음 (삽입위치,삽입값)
                    }else{
            ...
            => 처음부터 렌더링하는 애들은이벤트 걸어도 상관 x (hmtl로 작성된애들)
            이이벤트가 걸리는시점에선 얘가 완성되지 않은 상태일수도있기때문ㅇ ㅔ가상의 돔에다가는 이벤트 걸며면 동작이 안되 ㅁ
            
            .replyModify 요소는 실제 존재하는 요소가 아니라
            비동기 통신을 통해 생성되는 요소입니다
            .
            그러다보니 이벤트가 등록되는 시점보다 fetch함수의 실행이 먼저 끝날 거이라는 
            보장이 없기 때문에 해당 방식은 이벤트 등록이 불가능합니다.! 
            
            이 때는 이미 실제로 존재하는 #replyList 에 이벤트를 등록하고, 
            이벤트를 자식에게 위임하여 사용하는 addEventListener를 통해 처리해야 합니다. (이벤트 전파방식 ~ )
            ...
            */
            ...
            document.getElementById('modalModBtn').onclick = () =>{
                ...
                fetch('${pageContent.request.contextPath}/reply/' + rno, reqObj)
                .then(res => res.text())
                .then(data =>{
                    if(data === 'pwFail'){
                        alert('비밀번호를 확인하세요.');
            ...
            */
                ...
                const reqObj = {
                    ...
                    body:replyPw // 값이 하나만 () 이케만 보내도됨!
                    
                }

                fetch('${pageContent.request.contextPath}/reply/' + rno, reqObj)
                .then(res=> res.text()) //텍스트 주니까 text부른거 ~! 
                .then(data =>{
                    if(data === 'deleteSeccess'){
                        alert('정상 삭제')
                        document.getElementById('modalPw').value='';
                        $('#replyModal').modal('hide');
                        getList(1,true); 
            ...
            function parseTime(regDate){
                // 원하는 날짜로 객체를 생성 !
                const now = new Date(); // date 안에 지금날짜 有
                const regTime = new Date(regDate);
                
                // getTime(): 현재 시간을 1970년 1월 1일 자정을 기준으로 현재까지의 시간을 밀리초로 리턴.
                const gap = now.getTime() - regTime.getTime();
                let time;

                if(gap < 60 * 60 * 24 * 1000){ // gap < 하루보다 작냐 
                    if(gap<60 * 60 * 1000){ // 1시간보다도 작냐 
                        time = '방금 전';
                    }else{ // 1시간 넘냐 
