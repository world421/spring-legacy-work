	...
	<script>
		...
		function regist(){
			// 세션에서 현재 로그인 중인 사용자의 정보(아이디)를 얻어오자.
				const userId = '${sessionScope.login}'; // 세션에 있는 login 꺼내오자
				if(userId === ''){ //세션 데이터 없다 ? - > 로그인 X 
					alert('로그인이 필요한 서비스 입니다.')
					return
				} 

				// 자바 스크립트로 첨부한 파일 확장자 체크.
				let file = document.getElementById('file').value;
				console.log(file) // 중간경로 fakepath\ 로 로컬경로 가려줌 ! 

				//.을 제거한 확장자만 얻어낸 후 그것을 소문자로 일괄 변경!
				file = file.slice(file.indexOf('.')+1).toLowerCase();

				if(file !== 'jpg' && file !== 'png' && file !=='jepg' && file !== 'bmp'){
					alert('이미지 파일(jpg, png, jpeg, bmp) 만 등록 가능합니다.')
					return;
				}


				/*
				비동기 방식 요청에서 Form을 생성해서 보내주는 방법. (자바스크립트에서 사용방법 )
				-> FormData 객체를 활용합니다. 
				 (무조건 이 방식이 옳은 것은 아닙니다. FormData는 비동기 방식이 꼭 필요한 경우에만 사용합니다.)
				 대부분의 경우에는 form태그를 이용해서 전송하는 방식이 더 간편하고 더 자주 사용됩니다.
				*/

				const formData = new FormData(); // 내장 객체 ! 
				...
				// FormDate 객체에 사용자가 업로드한 파일의 정보가 들어있는 객체를 전달.
				//append('파라미터변수명' , '값');
				formData.append('file', $data.files[0]); //파일 정보 추가
				formData.append('content', document.getElementById('content').value);//글 내용추가
				...
				//file input 내용을 비워주시고, 글 영역 비워주시고, 
				//class 이름이 fileDiv (미리보기) 영역을 감춰 주세요. display 속성을 none 으로 .
		...
		$contentDiv.addEventListener('click', e=>{
			...
			fetch('${pageContext.request.contextPath}/snsboard/' + bno, { // 데이터없으면 body 에 내용 안담아도됨 ~! 
				method: 'delete'
			})
        ...
        */
		...
		$contentDiv.addEventListener('click', e =>{
			...
			const bno = e.target.getAttribute('href'); // 좋아요를 누른 글
			 // 아이디로 지목하면 안됨 !! 
			...
			.then(result=>{
				...
				if(result === 'like'){
					...
					e.target.firstElementChild.setAttribute('src', '${pageContext.request.contextPath}/img/like1.png')  // 지금 이벤트가 발생한 a태그 
