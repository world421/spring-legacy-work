...
public class UserController {
	...
	public String join(UserJoinRequestDTO dto, RedirectAttributes ra) {
		...
		// 모델은 forword에서 동작 ,redirect에서 사용 안함
		// add
	...
	public void login(String userId ,String userPw, Model model ) {
		System.out.println("나는 UserController의 login이당");
		model.addAttribute("result",service.login(userId, userPw));
	}
	
	// 마이페이지 이동 요청
	@GetMapping("/userMypage")
	public void userMypage(HttpSession session, Model model) { //이요청이 들어오면 HttpSession,modl주세요! 
		//마이페이지는 로그인 한 사람만 이동 가능 => 세션에 아이디가 없다.
